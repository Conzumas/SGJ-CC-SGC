...
The rewrite keeps the SGC concepts—persistent address book, dialing, gate telemetry, iris security, GDO/IDC authentication, and event logging—but uses Stargate Journey's current ComputerCraft peripheral API instead of JSG APIs.
...
Edit the CONFIG table at the top of `src/sgc.lua` before installing it. Set `transceiver_frequency` and `idc_code` if you want incoming GDO/IDC authentication. The incoming iris remains fail-closed when no valid IDC has been received.
